...
import { isAuth, handleResponse } from './authAction';
...
export const listBlogsWithCategoriesAndTags = async (skip, limit) => {
  ...
  try {
    const response = await fetch(`${API}/blogs-categories-tags`, {
      ...
      body: JSON.stringify(data), //* Sending skip and limit values from client to server
    });
    return await response.json();
  } catch (err) {
    return console.log(err);
...
export const listSearch = async params => {
  // console.log('blogsAction - listSearch - params: ', params);

  let query = queryString.stringify(params);

  // console.log('blogsAction - listSearch - query (params): ', query);
